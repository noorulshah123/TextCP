...
# -*- coding: utf-8 -*-
"""
Backwards-compatible Secrets Manager helper for:
  • SageMaker Domain (notebooks / pipelines)
  • ShinyProxy on ECS Fargate (on-demand + pre-init)

Storage convention (unchanged from your current lib):
  One "credentials" secret per scope that stores a JSON object of named entries.
  - Shared scope path:     /aap/<team>/shared/credentials
  - Individual user path:  /aap/<team>/<username>/credentials

Public methods (unchanged signatures where possible):
  - get_role_arn()
  - _secret_path(shared, secret_secret='credentials', username=None)
  - _secret_of_secrets(shared, username=None) -> dict
  - get_connection(name, shared=False, username=None) -> any
  - put_connection(name, secret_value, shared=False, username=None) -> response
  - delete_connection(name, shared=False, username=None) -> response
  - list_connections(shared=False, username=None) -> dict

Key improvements:
  - Robust team/env parsing from role names like:
      apps-<team>-<env>-app-ondemand-ecs-(task|execution)
      ...exec-domain-role-<team>
      ...exec-role-<team>
    If parsing fails, falls back to TEAM_NAME env.
  - Username resolution works for:
      SHINYPROXY_USERNAME          (on-demand seats)
      HTTP_X_SP_USERNAME / X_SP_USERNAME (if your server exports headers to env)
      explicit `username` argument  (recommended for pre-init seats; pass header
                                     X-SP-USERNAME from the request object)
"""
